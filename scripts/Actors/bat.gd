...
func _physics_process(_delta: float) -> void:
	...
	if not World.ray_intersects_ground(global_position, player):
		velocity = (player - global_position).normalized() * FLY_SPEED
		return
	...
	var target := _go_to_trajectory(path)

	# Check that the path is reachable
	if World.ray_intersects_ground(global_position, path[target]):
		navigation.restart(global_position)
		World.log("Actor cannot travel to path node ", path[target], ", clearing tree")
		return

	# We are already at the target, move to the next one
	if global_position.distance_to(path[target]) < DISTANCE_EPSILON \
		and target < path.size() - 1:
